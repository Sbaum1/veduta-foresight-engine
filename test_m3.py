...
parser.add_argument("--m3-path", type=str,
    default=r"V:\_staging\canonical\foresight_x\sample_data\m3\m3_monthly_dataset.tsf",
...
SAVE_EVERY = 25   # Save checkpoint every N series
...
def run():
    ...
    for i, s in enumerate(series_list):
        ...
        if done % SAVE_EVERY == 0 or (i + 1) == len(series_list):
            ...
            pct = (i + 1) / len(series_list) * 100
            print(f"  [{i+1:5d}/{len(series_list)}]  {pct:5.1f}%  "
                  f"elapsed={elapsed/60:.1f}m  "
                  f"median_MASE={med_mase}  "
                  f"errors={crashes + timeouts}  checkpoint saved")
