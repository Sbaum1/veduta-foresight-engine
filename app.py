...
"38 models. One answer. Elevated."

Updated: April 20, 2026
  · Model count updated: 22 → 38 (confirmed from registry.py v3.0.0)
  · M3 benchmark updated: MASE 0.6913 → 0.6847 (Foresight Engine v3.0.0,
    certified April 13, 2026, SHA-256: 0b980acd...7840e5c2)
...
def render_nav():
    ...
    with col_logo:
        ...
        <div style="padding:8px 0 6px 0">
            ...
            <div class="fx-tagline">38 Models &nbsp;·&nbsp; One Answer &nbsp;·&nbsp; Elevated</div>
        </div>""", unsafe_allow_html=True)
    ...
    with col_badge:
        ...
        <div class="m3-badge">
            ...
            M3 · MASE 0.6847 · #1 Modern Published
        </div>""", unsafe_allow_html=True)
        ...
        with st.expander("◈  Import Data", expanded=True):
            ...
            with col_in:
                ...
                raw = st.text_area(
                    "Paste series data",
                    height=200,
                    value=st.session_state.get("fx_sample_seed", ""),
                    placeholder=(
                        "2020-01-01, 248500\n"
                        "2020-02-01, 251300\n"
                        "...\n\n"
                ...
                with col_sample:
                    if st.button("Load Sample", key="fx_sample_btn",
                                 use_container_width=True):
                        st.session_state.fx_sample_seed = generate_sample_data()
                        st.rerun()
    ...
    if st.session_state.fx_data_committed:
        ...
        with st.expander("◈  Module 3 — Engine Configuration & Run", expanded=True):
            ...
            def engine_run_fragment():
                ...
                with col_info:
                    st.caption(
                        "38 models · MASE-inverse ensemble · "
                    ...
                    if result:
                        ...
                        <div class="fx-result-banner">
                            ...
                            <div class="fx-result-text">
                                ENGINE COMPLETE &nbsp;·&nbsp; {n_q}/38 qualified
                                &nbsp;·&nbsp; Top: {top}
                                &nbsp;·&nbsp; Switch to Tab II
                            </div>
                        </div>""", unsafe_allow_html=True)
...
def render_tab2():
    ...
    _section("Forecast Canvas", "Full-Engine Ensemble · Scenarios · Weighted MASE")
    ...
    for col, label, value, sub, color in [
        ...
        (c3, "Models Qualified",f"{n_qualified}/38",
    ...
    if hasattr(result, "regime_context") and result.regime_context.get("detected"):
        conf = result.regime_context.get("confidence", "medium")
        conf_color = "#C85A4A" if conf == "high" else "#E8A832"
        st.markdown(
            f"<div style='background:rgba(200,90,74,0.08);border:1px solid rgba(200,90,74,0.3);"
            f"border-radius:8px;padding:12px 18px;margin-bottom:16px'>"
            f"<div style='font-family:DM Mono,monospace;font-size:9px;letter-spacing:0.15em;"
            f"text-transform:uppercase;color:{conf_color};margin-bottom:4px'>"
            f"◈ Structural Break Detected — {conf.title()} Confidence</div>"
            f"<div style='font-family:DM Mono,monospace;font-size:9px;color:rgba(255,255,255,0.45);line-height:1.7'>"
            f"A regime change was detected in this series. Models that adapt to level shifts "
            f"(TBATS, Prophet, LocalLinearTrend) will rank higher. "
            f"Trend-following models may underperform on the new regime.</div>"
            f"</div>",
            unsafe_allow_html=True,
        )

    # ── Stacked Ensemble ───────────────────────────────────────────────────────
    if hasattr(result, "stacked_forecast") and result.stacked_forecast is not None:
        with st.expander("◈  Stacked Ensemble — Ridge Meta-Learner", expanded=False):
            st.markdown(
                "<div style='font-family:DM Mono,monospace;font-size:9px;"
                "color:rgba(255,255,255,0.4);line-height:1.8;margin-bottom:10px'>"
                "The Stacked Ensemble is a Ridge Regression meta-learner trained on "
                "out-of-sample fold forecasts from all base models. It learns which "
                "models to trust for this specific series type — rather than weighting "
                "by MASE alone. On well-behaved series it converges to the MASE-weighted "
                "ensemble. On complex series it may outperform significantly."
                "</div>",
                unsafe_allow_html=True,
            )
            stk = result.stacked_forecast
            ens = result.point_forecast
            diff_pct = float(np.mean((stk - ens) / (np.abs(ens) + 1e-8)) * 100)
            col_sk1, col_sk2, col_sk3 = st.columns(3)
            with col_sk1:
                st.markdown(_metric_card("Stacked Mean", f"{np.mean(stk):,.2f}", "Meta-learner output", "electric"), unsafe_allow_html=True)
            with col_sk2:
                st.markdown(_metric_card("Ensemble Mean", f"{np.mean(ens):,.2f}", "MASE-weighted ensemble", ""), unsafe_allow_html=True)
            with col_sk3:
                color = "teal" if diff_pct >= 0 else "red"
                st.markdown(_metric_card("Divergence", f"{diff_pct:+.2f}%", "Stacked vs ensemble", color), unsafe_allow_html=True)

    # ── Series intelligence ────────────────────────────────────────────────────
    if result.series_type or result.strategy:
        stype = result.series_type.value if result.series_type else "Standard"
        type_notes = {
            "Standard":     "Stable level with no dominant trend or seasonality. Most models qualify.",
            "Trended":      "Clear directional trend detected. Trend-adaptive models (LocalLinearTrend, Prophet, Theta) gain advantage.",
            "Seasonal":     "Strong seasonal pattern. STL+ETS, TBATS, DHR, SARIMA ranked higher.",
            "Intermittent": "High frequency of zero or near-zero values. Croston_SBA routed first.",
        }
        st.markdown(
            f"<div style='background:rgba(13,17,23,0.6);border:1px solid rgba(200,212,224,0.1);"
            f"border-radius:6px;padding:10px 16px;margin-bottom:14px'>"
            f"<span style='font-family:DM Mono,monospace;font-size:9px;letter-spacing:0.12em;"
            f"text-transform:uppercase;color:rgba(200,212,224,0.55)'>Series Classification: </span>"
            f"<span style='font-family:DM Mono,monospace;font-size:9px;color:#C8D4E0'>{stype}</span>"
            f"<span style='font-family:DM Mono,monospace;font-size:9px;"
            f"color:rgba(255,255,255,0.3)'> — {type_notes.get(stype, '')}</span>"
            f"</div>",
            unsafe_allow_html=True,
        )

    with st.expander("◈  Model Rankings", expanded=True):
        # Readiness tier color map
        tier_colors = {
            "Tier 1 — Production Ready":           "#4AB88A",
            "Tier 2 — Acceptable":                 "#E8A832",
            "Tier 3 — Weak":                       "#C85A4A",
            "Tier 3 — Weak (Limited Fold Validation)": "#C85A4A",
            "Tier 4 — Structural Failure":         "#C85A4A",
            "Ineligible — Minimum Data Not Met":   "rgba(255,255,255,0.2)",
            "Unscored":                            "rgba(255,255,255,0.2)",
            "Ineligible":                          "rgba(255,255,255,0.2)",
        }
        ...
        <div class="fx-model-header">
            ...
            <div style="min-width:68px;text-align:right">Composite</div>
            <div style="min-width:100px;padding-left:12px">Weight</div>
            <div style="min-width:120px;text-align:right">Readiness</div>
        </div>""", unsafe_allow_html=True)
        ...
        for r in result.rankings:
            ...
            comp_text   = f"{r.composite_score:.3f}" if not dq and r.composite_score < 99 else "—"
            ...
            tier      = getattr(r, "readiness_tier", "Unscored")
            tier_col  = tier_colors.get(tier, "rgba(255,255,255,0.2)")
            tier_short = (tier.replace("Tier 1 — ", "T1 ").replace("Tier 2 — ", "T2 ")
                             .replace("Tier 3 — ", "T3 ").replace("Tier 4 — ", "T4 ")
                             .replace("Ineligible — Minimum Data Not Met", "Ineligible")
                             .replace("Production Ready", "✓ Ready")
                             .replace("Structural Failure", "⚠ Failure"))[:18]
            ...
            <div class="fx-model-row">
                ...
                <div style="min-width:68px;text-align:right;font-family:'DM Mono',monospace;
                            font-size:9px;color:rgba(200,212,224,0.55)">{comp_text}</div>
                ...
                <div style="min-width:120px;text-align:right;font-size:9px;
                            font-family:'DM Mono',monospace;color:{tier_col}">
                    {tier_short}
                </div>
            </div>""", unsafe_allow_html=True)
...
def render_tab3():
    ...
    with st.expander("◈  M3 Benchmark Credential"):
        ...
        <div class="fx-credential">
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:20px">
                <div>
                    <div class="fx-credential-number" style="color:#C8974A">0.6847</div>
    ...
    # ── X-13 Information ───────────────────────────────────────────────────────
    with st.expander("◈  X-13 — What It Is and What It Does"):
        st.markdown("""
        <div style='font-family:DM Mono,monospace;font-size:9px;letter-spacing:0.12em;
                    text-transform:uppercase;color:rgba(200,212,224,0.6);margin-bottom:12px'>
            US Census Bureau X-13-ARIMA-SEATS · Diagnostic Only
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_x1:
            ...
            <div style='font-family:DM Mono,monospace;font-size:9px;
                ...
                <div style='color:#C8D4E0;margin-bottom:6px'>What X-13 is</div>
                X-13-ARIMA-SEATS is the official seasonal adjustment
                algorithm used by the US Census Bureau and most central
                banks worldwide. It decomposes a historical series into
                its trend, seasonal, and irregular components using a
                combination of ARIMA modeling and signal extraction.<br><br>
                It is the gold standard for seasonal adjustment — the
                same algorithm that produces seasonally adjusted GDP,
                employment, and manufacturing data published by the Fed.
            </div>
            """, unsafe_allow_html=True)
        with col_x2:
            st.markdown("""
            <div style='font-family:DM Mono,monospace;font-size:9px;
                        color:rgba(255,255,255,0.55);line-height:2'>
                <div style='color:#C8D4E0;margin-bottom:6px'>What X-13 does NOT do</div>
                X-13 does <span style='color:#C85A4A'>not produce a forward forecast</span>.
                It analyzes the past — it cannot project what comes next.
                For this reason it is registered as
                <span style='color:#E8A832'>diagnostic only</span> and does not
                participate in the ensemble or compete for ranking.<br><br>
                <div style='color:#C8D4E0;margin-top:8px;margin-bottom:6px'>
                    Why it runs anyway
                </div>
                When X-13 detects strong seasonality, SARIMA and DHR
                tend to rank higher. Its decomposition validates that
                the ensemble is reading seasonality correctly.
            </div>
            """, unsafe_allow_html=True)

    # ── GARCH Information ──────────────────────────────────────────────────────
    with st.expander("◈  GARCH — Volatility Intelligence"):
        st.markdown("""
        <div style='font-family:DM Mono,monospace;font-size:9px;letter-spacing:0.12em;
                    text-transform:uppercase;color:rgba(200,212,224,0.6);margin-bottom:12px'>
            GARCH(1,1) · AR(1) Mean · Conditional Heteroskedasticity
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_g1:
            ...
            <div style='font-family:DM Mono,monospace;font-size:9px;
                        color:rgba(255,255,255,0.55);line-height:2'>
                <div style='color:#C8D4E0;margin-bottom:6px'>Two outputs, two roles</div>
                GARCH produces two things simultaneously:<br><br>
                <span style='color:#4AB88A'>① Level forecast</span> — an AR(1) mean
                equation that captures serial dependence and produces a
                point forecast. This competes in the ensemble like any
                other model and is MASE-weighted accordingly.<br><br>
                <span style='color:#4A9EE0'>② Volatility forecast</span> — a
                conditional sigma per forecast horizon stored in model
                metadata. On calm periods intervals narrow. On volatile
                periods they widen. This is more honest than fixed-width
                confidence bands.
            </div>
            """, unsafe_allow_html=True)
        with col_g2:
            ...
            <div style='font-family:DM Mono,monospace;font-size:9px;
                        color:rgba(255,255,255,0.55);line-height:2'>
                <div style='color:#C8D4E0;margin-bottom:6px'>When GARCH ranks high</div>
                GARCH performs best on series with
                <span style='color:#E8A832'>volatility clustering</span> — periods
                of calm followed by periods of turbulence. Common in:<br><br>
                · Commodity-driven revenue (oil, metals, agriculture)<br>
                · Series with supply chain disruptions<br>
                · Financial or macro-sensitive manufacturing output<br>
                · Any series where COVID or other shocks created
                  regime changes in variance<br><br>
                If GARCH ranks in the top 5, your series has meaningful
                volatility structure that linear models miss.
            </div>
            """, unsafe_allow_html=True)
    ...
    with st.expander("◈  Model Audit" + (" — Full Detail" if source_mode else "")):
        ...
        for r in result.rankings:
            tier    = getattr(r, "readiness_tier", "Unscored")
            posture = getattr(r, "confidence_posture", "—")
            flags   = getattr(r, "risk_flags", [])
            rows.append({
                "Rank":       r.rank,
                "Model":      r.model_id,
                "MASE":       round(r.mase, 4),
                "SMAPE":      round(r.smape, 4) if hasattr(r, "smape") and r.smape and not np.isnan(r.smape) else "—",
                "Composite":  round(r.composite_score, 4) if hasattr(r, "composite_score") and r.composite_score < 99 else "—",
                "Weight":     round(r.ensemble_weight, 4) if r.ensemble_weight else 0,
                "Readiness":  tier,
                "Posture":    posture if not r.disqualified else "—",
                "Risk Flags": "; ".join(flags[:2]) if flags else "—",
