...
class _ForesightEngineAdapter:
    ...
    def run(self, fi, run_id: str = "") -> _ForesightResult:
        ...
        for name, result in raw.items():
            if name.startswith("_") or name == "Primary Ensemble":
                continue
    ...
    def _build_rankings(self, raw: Dict[str, Any]) -> List[_RankingEntry]:
        ...
        for name, result in raw.items():
            ...
            if status != "success" or mase is None:
                ...
                rows.append({
                    "name":   name,
                    "mase":   float(mase),
                    "smape":  float(smape) if smape is not None else float("nan"),
                    "weight": 0.0,
                    "dq":     False,
                    "reason": None,
                })
        ...
        # Inverse-MASE weights for ensemble members
        mase_vals = np.array([r["mase"] for r in success], dtype=float)
